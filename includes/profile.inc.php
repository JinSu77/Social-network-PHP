...
class Profile
{
    ...
    public function DeleteAllCommente()
    {
        $request = $this->bdd->prepare("DELETE FROM reaction_comment where user_id= ?");
        $request->execute([$_SESSION["userid"]]);
    }
    public function DeleteAllLike()
    {
        $request = $this->bdd->prepare("DELETE FROM reaction_like where user_id= ?");
        $request->execute([$_SESSION["userid"]]);
    }
