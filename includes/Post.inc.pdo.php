...
class Post {
    ...
    public function getPostbyIdPost($id){
        $request = $this->prepare("SELECT Post.id FROM Post INNER JOIN users ON Post.user_id = users.id WHERE Post.id = ? ");
        $request-> execute([$id]);
        $resultat = $request->fetchAll(PDO::FETCH_ASSOC);
    }
    //recuper les 3 dernier post de l'utilisateur
    public function  getLastPost($user_id) {
        $request = $this->prepare("SELECT users.id FROM Post INNER JOIN users ON Post.user_id = users.id WHERE users.id = ? ORDER BY Post.post_date  DESC LIMIT 3");
        $request-> execute([$id]);
        $resultat = $request->fetchAll(PDO::FETCH_ASSOC);
    }
